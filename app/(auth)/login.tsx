...
// Wajib ditaruh di luar komponen agar browser native otomatis tutup setelah login OAuth
...
export default function LoginScreen() {
  ...
  // Mengambil state dan method dari authStore kita yang sudah hybrid
  ...
  // Validasi login: pastikan email tidak kosong & password terisi
  const isEmailValid = email.trim().length > 0
  const isPasswordValid = password.length > 0
  const isFormValid = isEmailValid && isPasswordValid

  // --- 1. HANDLE LOGIN HYBRID (SUPABASE CLOUD / LOCAL SQLITE) ---
  const handleLogin = async () => {
    if (!isFormValid) return

    try {
      await login(email.trim(), password)
      // Kalau sukses (baik akun cloud maupun akun lokal), langsung pindah ke dashboard utama
      router.replace('/(tabs)')
    } catch (err: any) {
      // Error otomatis tersimpan di dalam state 'error' di authStore
      Alert.alert('Login Gagal', err.message || 'Email atau password salah!')
    }
  }

  // --- 2. HANDLE SOCIAL LOGIN (ONLY FOR PREMIUM USERS) ---
  const handleSocialLogin = async (provider: string) => {
    ...
    try {
      ...
      // Memanggil method Google OAuth dari store yang sudah ada proteksi Gatekeeper-nya
      await loginWithGoogle()
      // Note: Redirect setelah login OAuth akan ditangani otomatis oleh listener di _layout / restoreSession
    } catch (err: any) {
      console.error('❌ Google Login Error:', err)
      Alert.alert('Gagal Login Google', err.message || 'Terjadi kesalahan saat masuk dengan Google.')
    } finally {
      ...
      >
        ...
        >
          ...
          <View style={styles.footer}>
            ...
            <Pressable onPress={() => router.push('/(auth)/register')} disabled={loading || isSocialLoading}>
